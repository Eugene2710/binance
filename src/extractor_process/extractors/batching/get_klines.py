...
class BinanceKlinesExtractor:
    ...
    async def extract(
        ...
        """
        base: str = "https://api.binance.com/api/v3/klines"
        ...
        async with aiohttp.ClientSession() as client:
            async with client.get(url, ssl=False) as response:
                if response.status == 200:
                    data: list[list[str | int]] = await response.json()
...
if __name__ == "__main__":
    ...
    res: Klines = event_loop.run_until_complete(
        binance_klines_extractor.extract(
            symbol="BTCUSDT",
            interval="1m",
